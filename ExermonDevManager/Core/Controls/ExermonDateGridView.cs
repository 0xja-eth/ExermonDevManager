...
namespace ExermonDevManager.Core.Controls {
	...
	public partial class ExermonDataGridView : DataGridView {
		...
		public Action<PropertyInfo, CoreData> onEditCell;
		...
		public void setItems(CoreData root, PropertyInfo prop, BindingSource source) {
		...
		DataGridViewColumn createColumn(PropertyInfo prop, 
			...
			else if (type.IsSubclassOf(typeof(CoreData))) // 下拉框
		...
		DataGridViewColumn createComboxCol(PropertyInfo prop, Type type) {
			...
			res.DataSource = DataManager.getDataList(type);
			res.DisplayMember = "displayName";
			res.ValueMember = "id";
		...
		/// <returns></returns>
		public CoreData currentItem() {
			var source = DataSource as BindingSource;
			return source.Current as CoreData;
		}
		...
		/// <typeparam name="T"></typeparam>
		/// <returns></returns>
		public T currentItem<T>() where T : CoreData {
			var source = DataSource as BindingSource;
